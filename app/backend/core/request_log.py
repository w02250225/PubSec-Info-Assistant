...
class RequestLog:
    ...
    def encode_document_id(self, session_id: str, conversation_id: str):
        """ Encode a string to remove unsafe chars for a cosmos db id """
        document_id = session_id + conversation_id
    ...
    async def log_request(self,
        ...
        document_id = self.encode_document_id(session_id, conversation_id)
    ...
    async def log_response(self,
        ...
        try:
            ...
            json_document = self._log_document[log_id]
            ...
            self._log_document.pop(log_id, None)
    ...
    async def get_conversation_history(self, user_id):
       ...
       c.conversation_id,
       c.conversation_name,
       c.history[0].start_timestamp AS conversation_start
FROM   c
...
ORDER BY c.history[0].start_timestamp DESC
"""
    ...
    async def get_conversation(self, conversation_id):
        ...
        query = f"SELECT c.history FROM c WHERE c.conversation_id = '{conversation_id}'"
        ...
        return json.dumps(chat_app_responses, indent=2)
