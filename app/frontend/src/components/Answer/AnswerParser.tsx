...
export function parseAnswerToHtmlExport(answer: string, citation_lookup: CitationLookup): HtmlExportParsedAnswer {
    const citations: string[] = [];
    const sourceFiles: Record<string, string> = {};
    const pageNumbers: Record<string, number> = {};
    
    // Extract any follow-up questions that might be in the answer
    let parsedAnswer = answer.replace(/<<<([^>>>]+)>>>/g, "");
    ...
    const fragments: string[] = parts.map((part, index) => {
        if (index % 2 === 0) {
            // Even parts are just text
            return part;
        } else { // Odd parts are citations as the "FileX" moniker
            const citation = citation_lookup[part];
            ...
            return renderToStaticMarkup(<sup>{citationIndex}</sup>);
            }
        }
