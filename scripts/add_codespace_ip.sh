...
if [ "$OLD_IP_ADDR" != "$CURR_IP_ADDR" ]; then
    ...
    az webapp config access-restriction remove -g rgdsibinfoasstazs01 -n infoasst-web-kr839 --rule-name Codespace || true
    az webapp config access-restriction add -g rgdsibinfoasstazs01 -n infoasst-web-kr839 --rule-name Codespace --action Allow --ip-address $CURR_IP_ADDR --priority 250 || true

    az webapp config access-restriction remove -g rgdsibinfoasstazs01 -n infoasst-enrichmentweb-kr839 --rule-name Codespace || true
    az webapp config access-restriction add -g rgdsibinfoasstazs01 -n infoasst-enrichmentweb-kr839 --rule-name Codespace --action Allow --ip-address $CURR_IP_ADDR --priority 250 || true
    ...
    az cognitiveservices account network-rule add --name CADISOPENAIAZS01 --resource-group RGDOPENAIAZS01 --ip-address $CURR_IP_ADDR || true
