...
REDIRECT_URI="https://infoasst-web-$RANDOM_STRING.$WEB_APP_ENDPOINT_SUFFIX/authorized"
...
if [ -n "${IN_AUTOMATION}" ]; then
    ...
    if [ -z $aadWebAppId ]
    then
        aadWebAppId=$(az ad app create --display-name infoasst_web_access_$RANDOM_STRING --sign-in-audience AzureADMyOrg --identifier-uris "api://infoasst-$RANDOM_STRING" --web-redirect-uris $REDIRECT_URI --enable-access-token-issuance true --enable-id-token-issuance true --output tsv --query "[].appId")
        aadWebAppId=$(az ad app list --display-name infoasst_web_access_$RANDOM_STRING --output tsv --query [].appId)
    fi
    aadWebAppIdSecret=$(az ad app credential reset --id $aadWebAppId --display-name infoasst-web --output tsv --query password)
...
export AZURE_AD_WEB_APP_SECRET=$aadWebAppIdSecret
...
declare -A REPLACE_TOKENS=(
    ...
    [\${AZURE_AD_WEB_APP_SECRET}]=${AZURE_AD_WEB_APP_SECRET}
)
